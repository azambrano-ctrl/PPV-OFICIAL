...
export default function AdminEditFighter() {
    ...
    useEffect(() => {
        ...
        if (params.slug) {
            loadFighterProfile(params.slug as string);
        }
    }, [isAuthenticated, user, params.slug]);

    const loadFighterProfile = async (slug: string) => {
        try {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await fightersAPI.updateAsAdmin(fighterId, submitData);
            ...
            <div className="flex-1 py-12 pt-32">
                <div className="container-custom max-w-4xl">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        <Link href="/admin/fighters" className="p-2 bg-dark-800 rounded-lg hover:bg-dark-700 transition">
                            <ArrowLeft className="w-5 h-5 text-dark-300" />
                        </Link>
                        <div>
                            <h1 className="font-display text-4xl font-bold">
                                Edición de <span className="gradient-text">Peleador</span> (Admin)
                            </h1>
                            <p className="text-dark-400">
                                Estás modificando la ficha oficial del peleador.
                            </p>
                        </div>
                    </div>
